...
def favicon():
    ...
    return send_from_directory(
        str(BASE_DIR / "static"), "favicon.svg", mimetype="image/svg+xml"
    )
...
def favicon_cube_map():
    return send_from_directory(
        str(BASE_DIR / "cube_map" / "static"), "favicon.svg", mimetype="image/svg+xml"
    )
...
def api_model():
    ...
    if not MODEL_FILE.exists():
        return jsonify(
            {"error": "Model cache not found — POST /api/refresh to extract"}
        ), 404
    with open(MODEL_FILE, encoding="utf-8") as f:
        model = json.load(f)
    # Inject _tags from tags.json if not embedded in model yet
    if "_tags" not in model and TAGS_FILE.exists():
        try:
            model["_tags"] = json.loads(TAGS_FILE.read_text())
        except Exception:
            pass
    ...
    def do_refresh():
        with _refresh_lock:
            ...
            try:
                ...
                model = extract_model()
                # Preserve tags from the old model into the new one
                if TAGS_FILE.exists():
                    try:
                        old_tags = _json.loads(TAGS_FILE.read_text())
                        model["_tags"] = old_tags
                    except Exception:
                        pass
...
def api_servers():
    """List all configured TM1 instances and their databases."""
    try:
        from core.tm1_connect import load_servers, _active_instance_idx, _active_db_idx

        servers = load_servers()
        return jsonify(
            {
                "servers": servers,
                "activeInstance": _active_instance_idx,
                "activeDb": _active_db_idx,
            }
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
@app.route("/api/servers/active", methods=["POST"])
def api_servers_set_active():
    """Switch active TM1 instance and database. Body: {instance: int, db: int}"""
    try:
        from core.tm1_connect import set_active_profile, invalidate_session

        data = request.get_json(force=True)
        instance_idx = int(data.get("instance", 0))
        db_idx = int(data.get("db", 0))
        set_active_profile(instance_idx, db_idx)
        invalidate_session()
        log.info("Active server: instance=%d db=%d", instance_idx, db_idx)
        return jsonify({"status": "ok", "instance": instance_idx, "db": db_idx})
    except Exception as e:
        log.error("Set active server error: %s", e)
        return jsonify({"error": str(e)}), 500
...
def api_tags_save():
    ...
    # Also persist tags into the model file so they survive refresh
...
def api_performance_monitor():
    ...
    try:
        ...
        if request.method == "GET":
            ...
            r = session.get(
                f"{session.base_url}/Cubes?$filter=Name eq '}}StatsByCube'&$select=Name"
            )
        ...
        if not supported:
            return jsonify(
                {"error": "Performance Monitor not supported on this server"}
            ), 501
        ...
        for view in r.json().get("value", []):
            ...
            for axis in ("Rows", "Columns", "Titles"):
                for entry in view.get(axis) or []:
                    if (
                        entry.get("DimensionName", "").lower() == dim_lc
                        and entry.get("SubsetName", "").lower() == subset_lc
                    ):
                        found = True
                        break
                if found:
...
def api_tm1_mdx():
    ...
    try:
        ...
        session = get_session()
        r = session.get(
            f"{session.base_url}/Cubes('{cube}')/Views('{view}')?$select=Name,MDX"
        )
        r.raise_for_status()
        data = r.json()
        mdx = data.get("MDX") or ""
...
def api_spec_get(obj_id):
    ...
    return jsonify(
        {"exists": True, "spec": json.loads(spec_file.read_text(encoding="utf-8"))}
    )
...
def _build_spec_prompt(obj_id: str) -> str:
    ...
    if obj_type == "cube":
        ...
        if script_path and Path(script_path).exists():
            ...
            context_lines.append(
                Path(script_path).read_text(encoding="utf-8", errors="replace")
            )
        else:
...
def api_script_python():
    ...
    if not sources_file.exists():
        return jsonify(
            {"status": "error", "message": "python_sources.json not found"}
        ), 404

    registered = {
        s["path"] for s in json.loads(sources_file.read_text()) if "path" in s
    }
    if path_param not in registered:
        return jsonify(
            {"status": "error", "message": "Script not in registered sources"}
        ), 403
    ...
    return jsonify(
        {"status": "ok", "content": p.read_text(encoding="utf-8"), "path": path_param}
    )
...
def api_script_ti():
    ...
    try:
        ...
        content = "\n\n".join(
            f"# ── {sec} ──────────────────────\n{code}"
            for sec, code in sections.items()
            if code.strip()
        )
...
def _build_module_prompt(tags: list) -> str:
    ...
    cubes = model.get("cubes", {})
    tags_data = (
        json.loads(TAGS_FILE.read_text(encoding="utf-8")) if TAGS_FILE.exists() else {}
    )
    cube_tags = tags_data.get("cubeTags", {})

    tag_set = set(tags)
    module_ids = {
        oid for oid, otags in cube_tags.items() if any(t in tag_set for t in otags)
    }
    ...
    for obj_id in sorted(module_objs.keys()):
        ...
        if obj_type == "cube":
            dims = [d["n"] if isinstance(d, dict) else d for d in c.get("dims", [])]
            if dims:
...
if __name__ == "__main__":
    ...
    log.info(
        f"  Model cached: {'✅ yes' if MODEL_FILE.exists() else '⚠️  no — refresh to extract'}"
    )
    log.info("══════════════════════════════════════════")
